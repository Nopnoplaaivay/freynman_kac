...
# ---------------------------
# Fourier Neural Operator Price Prediction Function
# ---------------------------
def fourier_neural_operator_price(S0, K, r, sigma, T, **kwargs):
    """
    Predict the option price at S = S0 for a given strike K using the Fourier Neural Operator.
    
    Hyperparameters (via kwargs):
      - num_samples: number of training samples (default 1000)
      - epochs: number of training epochs (default 5000)
      - lr: learning rate (default 1e-3)
      - m: number of discretization points for the spatial grid (default 100)
      - modes: number of Fourier modes (default 16)
      - width: width (number of channels) (default 64)
      - layers: number of Fourier layers (default 4)
    ...
    model, S_grid = train_fno(num_samples=num_samples, epochs=epochs, lr=lr,
                              m=m, modes=modes, width=width, layers=layers,
                              S_max=S_max, r=r, sigma=sigma, T=T)
    # For the given strike K, compute the terminal payoff function on S_grid
    g = np.maximum(S_grid - K, 0).reshape(1, -1)  # shape (1, m)
    X_test = torch.tensor(g, dtype=torch.float32)
    model.eval()
    with torch.no_grad():
        pred = model(X_test)  # shape (1, m)
    pred = pred.squeeze(0).cpu().numpy()  # predicted solution function V(S) at t=0
    # Interpolate to get the price at S0
    f_interp = interp1d(S_grid, pred, kind='linear', fill_value="extrapolate")
    price = f_interp(S0)
    return float(price)
